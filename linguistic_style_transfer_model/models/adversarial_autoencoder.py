...
class AdversarialAutoencoder:
    ...
    def build_model(self, word_index, encoder_embedding_matrix, decoder_embedding_matrix, num_labels):
        ...
        self.conditioning_embedding = tf.placeholder(
            dtype=tf.float32, shape=[None, model_config.style_embedding_size],
    ...
    def get_batch_indices(self, batch_number, data_limit):
        ...
        end_index = min((batch_number + 1) * model_config.batch_size, data_limit)
    ...
    def train(self, sess, data_size, padded_sequences, text_sequence_lengths, one_hot_labels, num_labels,
        ...
        self.composite_loss -= self.bow_entropy * model_config.adversarial_bow_loss_weight
        ...
        if data_size % model_config.batch_size:
            num_batches += 1
        ...
        for current_epoch in range(1, options.training_epochs + 1):
            ...
            for batch_number in range(num_batches):
                ...
                [_, _, _, reconstruction_loss, adversarial_loss, adversarial_entropy,
                 style_loss, bow_prediction_loss, bow_entropy, composite_loss,
                ...
                logger.info(log_msg.format(
                    reconstruction_loss, adversarial_loss, adversarial_entropy, style_loss, bow_prediction_loss,
                    bow_entropy, current_epoch, batch_number, composite_loss))
            ...
            if not current_epoch % global_config.validation_interval:
                ...
                for i in range(num_labels):
                    ...
                    if len(validation_sequences_to_transfer) % model_config.batch_size:
    ...
    def generate_novel_sentences(self, sess, padded_sequences, text_sequence_lengths, style_embedding,
        ...
        if len(padded_sequences) % model_config.batch_size:
            num_batches += 1
